...
def test_inject_colors_prepends_style_block_before_close_head():
    html = "<!DOCTYPE html><html><head><title>x</title></head><body></body></html>"
    out = inject_colors(html, "#FF6B00", "#1A1A1A", "#0F0F0F")
    expected = "<style>:root{--primary-color:#FF6B00;--accent-color:#1A1A1A;--bg-color:#0F0F0F;}</style></head>"
    assert expected in out

def test_inject_colors_only_replaces_first_close_head():
    html = "<head></head><head></head>"  # pathological double-head
